...
fixed_size_list_from_erlang(_Config) ->
    Type = from_erlang(arrow_fixed_list_array:from_erlang([[1, 2, 3]], {s, 8})),
    ?assertEqual(Type, {fixed_size_list, #{list_size => 3}}).

large_binary_from_erlang(_Config) ->
    Type = from_erlang(
        arrow_variable_binary_array:from_erlang([<<1>>, <<1, 2>>, <<1, 2, 3>>], {s, 8})
    ),
    ?assertEqual(Type, large_binary).

large_list_from_erlang(_Config) ->
    Type = from_erlang(
        arrow_variable_list_array:from_erlang([[1], [1, 2], [1, 2, 3]], {s, 8})
    ),
...
fixed_primitive(Type) ->
    from_erlang(arrow_fixed_primitive_array:from_erlang([1, 2, 3], Type)).

from_erlang(Array) ->
    arrow_ipc_type:from_erlang(Array).
